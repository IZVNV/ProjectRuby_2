# Création de l'array de handles
...
puts "Le handle le plus court de cette liste est #{handle_plus_court}."

# Combien y a-t-il de handle contenant 5 caractères (le @ ne compte pas pour un caractère) ?
nombre_handles_5_caracteres = handles.count { |handle| handle.length == 6 }
puts "Il y a #{nombre_handles_5_caracteres} handles contenant 5 caractères."

# Combien commencent par une majuscule (première lettre juste après le @) ?
nombre_handles_majuscule = handles.count { |handle| handle[1] =~ /[A-Z]/ }
puts "Il y a #{nombre_handles_majuscule} handles commençant par une majuscule."

# Trie la liste de handles par ordre alphabétique.
handles_tries_alphabetique = handles.sort_by { |handles| handles.downcase }
puts "Liste de handles triée par ordre alphabétique :"
puts handles_tries_alphabetique

# Trie la liste de handles par taille des handles (les plus petits en premiers, les plus grands après).
...
puts "La personne @epenser est à la position #{position_epenser} dans l'array."

# Sors-moi une répartition des handles par taille de ces derniers (nombre de handles avec 1 caractère, nombre de handles avec 2 caractères, nombre de handles avec 3 caractères, etc.)

    number_of_items =0
    chractere_number = (1..15)
    chractere_number.each do |charactere|
        number_of_items = handles.count {|handel| handel.size == charactere}
        puts "nombre de handle avec #{charactere -1} caractère #{number_of_items}"
end